      ...
      {open && (
        <div className="absolute z-50 mt-2 w-full overflow-hidden rounded-xl border bg-popover/95 shadow-2xl shadow-black/40 backdrop-blur-2xl">
          ...
          <div className="max-h-72 overflow-y-auto p-1.5">
            <Item label={t('ns.all')} active={selected === ''} onClick={() => { onSelect(''); setOpen(false) }} />
            {filtered.map((n) => (
              <Item key={n.name} label={n.name} active={selected === n.name} onClick={() => { onSelect(n.name); setOpen(false) }} />
            ))}
          </div>
        </div>
      )}
